...
#         return output, alphas
    ...
    def create_model_graph(self):
        ...
        conv = tf.layers.conv1d(self.b_input, self.num_filters, self.kernel_size, name='conv')#卷积层里面的参数有词向量输入，过滤器的数量，卷积核的大小，和这个层的名字
        # global max pooling layer
        gmp = tf.reduce_max(conv, reduction_indices=[1], name='gmp')#reduce_max就是池化层的一种
        ...
        self.logit_class = tf.layers.dense(fc, units=self.config.feature_dim[self.feature_index])
