...
@description: 构建全连接层
@argvs:  name - 全连接层的名称
         input_dim - input数据的维度 128
         output_dim - output数据的维度 sequence_length * dim
         inputs - 数据
         biases - 是否添加偏置项
         initialization - 变量的初始化方式
         weightnorm - 权重的初始化方式
         gain -
"""
...
def Linear(
    ...
    with tf.name_scope(name) as scope:
        ...
        # weight_values -> input_dim * output_dim , 128 * sequence_length * dim
        weight_values *= gain
        ...
        if inputs.get_shape().ndims == 2:
            ...
            result = tf.reshape(result, tf.pack(
                tf.unpack(tf.shape(inputs))[:-1] + [output_dim]))
        ...
        # result -> batch_size * sequence_length * dim
        return result
