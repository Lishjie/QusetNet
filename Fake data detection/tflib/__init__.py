...
@author: lishijie
@date: 2019/12/04
@description: 用于模型的变量复用和参数打印
"""
import numpy as np
import tensorflow as tf

# 用于根据当前计算机用户所处的语言环境，本地化转换程序所展示的语言
import locale

# 将所有的locale设置成用户默认的设置（通常是环境变量LANF值）
# 该方法不能保证线程安全性
# local categorg 决定字符处理函数的相关行为，比如string函数
...
def param(name, *args, **kwargs):
    ...
    if name not in _params:
        kwargs['name'] = name                 # 设定name参数的值
        param = tf.Variable(*args, **kwargs)  # 创建一个新的变量  *args初始值  **kwargs其他参数设置
        param.param = True
        _params[name] = param                 # 将新的变量添加到变量字典中
    result = _params[name]                    # 从变量字典中获取到已有的变量
    i = 0
    # 匹配字典的key值
    # 获取变量字典的参数
