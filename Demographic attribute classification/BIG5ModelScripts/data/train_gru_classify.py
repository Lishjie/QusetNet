...
#    best_b_acc = 0.0  # ji lu hou xiang de zui gao jing du
    ...
    with tf.Graph().as_default(), tf.Session(config=config_tf) as session:
        ...
        with tf.variable_scope("model", reuse=None, initializer=initializer):
            m = Model.Model(config, feature_index, concat_mode='add', is_training=True, add_attention=False)
        
        with tf.variable_scope("model", reuse=True, initializer=initializer):
            mValidate = Model.Model(config, feature_index, concat_mode='add',is_training=False,add_attention=False)
        ...
        for i in range(config.iteration):
            ...
            if max_accuracy < np.mean(accuracys_list):
                ...
                model_saver.save(session, config.path+'trained-model/model-gru-concat'+str(feature_index)+'/'+'lm-%d' % (i+1))
                print('Done!')
            else:
