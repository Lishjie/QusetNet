...
abnormal_index = [2717, 22298, 23544, 24280, 26793, 42012, 44165, 65236, 93044]
abnormal_answer = []
abnormal_feature = []

for i, val in enumerate(abnormal_index):
    abnormal_answer.append(answers.iloc[val, 0:].to_numpy())
    abnormal_feature.append(feature.iloc[val, 0:].to_numpy())
np.save("abnomal_answer.npy", abnormal_answer)
np.save("abnomal_feature.npy", abnormal_feature)
...
for i in range(answers_np.shape[1] - 1):
    ...
    length += data.shape[0]
    len_max += np.array(np.where((data[:]>2.06) & (data[:]<=2.50))).shape[1] # 强相反问题的个数
    len_min += np.array(np.where((data[:]>=0) & (data[:]<1.46))).shape[1] # 强相关问题的个数
...
# 根据一定的规则(mse的差值范围)获取出问题关系矩阵中的强相关和强相反问题
strong_relation = np.full((50, 50), -1.0, )  # 初始化强相关问题矩阵
for row, row_val in enumerate(problem_mse_array):
    for col, val in enumerate(row_val):
        if val == -1:
            continue
        elif val > 2.06 and val <= 2.50 or val >= 0 and val < 1.46:
            strong_relation[row][col] = val
